...
const yOff = 0.0015;
const zOff = 0.0015;
const backgroundColor = "hsla(0,0%,90%,1)";
...
function initCircle(i: number) {
  ...
  hue = Math.abs((baseHue * n * rangeHue) % 50) + 200;
  // console.log(hue);
...
function drawCircle(
  ...
  ctx.a.save();
  ctx.a.fillStyle = `hsla(${hue},50%,30%,${fadeInOut(life, ttl)})`;
  ctx.a.beginPath();
